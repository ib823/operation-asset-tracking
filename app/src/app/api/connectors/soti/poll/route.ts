...
const PollBody = z.object({
   ...
   * adapter (Phase 2) fetches these from the MDM and ignores any body.
   */
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (!flags.soti) {
    // Not an error: a deployment without SOTI is a supported configuration, and the
    // register works without it. Say so plainly rather than failing.
